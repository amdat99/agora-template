...
const App: React.FC = () => {
  ...
  const [auth, setAuthenticated] = useState<any>(null);

  React.useEffect(() => {
    const urlSplit = window.location.href.split('?');
    const user = urlSplit[1].split('&')[0].split('=')[1];
    const org = urlSplit[1].split('&')[1].split('=')[1];
    const name = urlSplit[1].split('&')[2].split('=')[1];
    setAuthenticated({user, org, name});
    console.log(auth);
